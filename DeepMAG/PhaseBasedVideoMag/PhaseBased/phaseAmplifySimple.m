function phaseAmplifySimple(vidFile, alpha, fl, fh, outFile, pyrFile, varargin)
...
filt = numOrients-1;
[pyr,pind,steermtx,harmonics] = buildSCFpyr(seq(:,:,1,1), numLevels, filt);
...
pyrRef = buildSCFpyr(seq(:,:,1,refFrame), numLevels, filt, twidth);

parfor ii = ceil(numFrames*0.3):ceil(numFrames*0.7)
...
savepyr = cell(1,numFrames);
% for ii = 1:numFrames
for ii = ceil(numFrames*0.3):ceil(numFrames*0.7)
    ii
...
%keyboard
%savepyr = cell2mat(savepyr);
save (pyrFile , 'savepyr') ;
% save(['\\vibe15\PublicAll\t-moali\E4\renamed\XYDeepMag',num2str(fileNum),'.mat'],'dXsub','dysub','-v7.3');

%writecell(savepyr, pyrFile)
vw.close;
