...
vidOutPath = "\\vibe15\PublicAll\t-moali\videos\15fps\256\pd\segmented-pd\deepMagPyr\";
...
for i = 1:numofFiles
    ...
    if files(i).isdir == 0
        ...
        if process == true && contains(files(i).name, ".mat")==false && contains(files(i).name, ".webm_preview")==true && contains(files(i).name, "task1")==false && contains(files(i).name, "task6")==false
