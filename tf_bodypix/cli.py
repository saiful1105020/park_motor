...
class AbstractWebcamFilterApp(ABC):
    ...
    def run(self):
        ...
        output_file_name = Path(self.args.source).name
        name_without_ext = Path(self.args.source).stem

        if not os.path.exists(out_parent):
            os.makedirs(out_parent)

        out_dir = os.path.join(out_parent,name_without_ext)
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)

        output_video_path = os.path.join(out_parent,output_file_name)
        #print("Checking parent of out:",out_path,out_parent)
        
        try:
            self.timer.start()
            counter = 0
            while True:
                status, output_image = self.next_frame()
                if status==False:
                    break
                im = Image.fromarray(output_image.astype('uint8'), 'RGB')
                im.save(os.path.join(out_dir, str(counter)+".jpg"))
                #im.save("C:/Wasif/PD Motor Feature Extraction/Facebook Body Segmentation/dino-main/output/temp%d.jpg"%(counter))
                counter +=1

            #Save video and delete image folder
            img_array = []
            for filename in natsort.natsorted(glob.glob(out_dir+"/*.jpg")):
                #print(filename)
